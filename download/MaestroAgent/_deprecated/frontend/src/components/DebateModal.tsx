        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {participants.map((p) => <span key={p} className="badge-info font-mono text-xs">{p}</span>)}
            </div>
          </div>
          <div>
            <label className="block text-xs text-ink-low uppercase tracking-wide mb-1.5">Debate topic</label>
            <textarea value={topic} onChange={(e) => setTopic(e.target.value)}
              placeholder="e.g. Should we use Postgres or MySQL for this SaaS?" rows={3}
              className="input w-full resize-none" />
          </div>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={seekConsensus}
              onChange={(e) => setSeekConsensus(e.target.checked)} className="accent-maestro-500" />
            <span className="text-sm text-ink-mid">Seek consensus (require quorum; otherwise escalate)</span>
          </label>
