...
export default function LoopsPanel() {
  ...
  // Refresh live state every 3s while a run is active.
  useInterval(
  ...
  const loops = useMemo(() => {
    ...
    events.forEach((e) => {
      ...
      if (e.type === "loop.iteration") {
        ...
        if (iter > byId[loopId].iterations) {
          byId[loopId].iterations = iter;
        }
        if (e.payload.score !== undefined) {
          byId[loopId].lastScore = e.payload.score as number;
        }
        if (e.payload.condition_reason) {
          byId[loopId].conditionReason = e.payload.condition_reason as string;
        }
  ...
  return (
    <div className="h-full flex flex-col gap-4 overflow-hidden">
      {/* Header with summary + create button */}
          ...
          {loops.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-xs text-ink-low mt-1">
                Loops appear here when an agent enters a verifiable cycle.
                <br />
...
function LoopCard({
    ...
    switch (loop.outcome) {
      case "exit_condition_met":
        return {
          ...
          outcomeLabel: "condition met",
          outcomeColor: "text-accent-ok",
        };
      case "stagnant":
      case "budget_exhausted":
        return {
          outcomeIcon: <AlertTriangle className="w-4 h-4 text-accent-warn" />,
          outcomeLabel: loop.outcome.replace("_", " "),
          outcomeColor: "text-accent-warn",
        };
      case "max_iterations":
      case "error":
        return {
  ...
  const scoreColor = loop.lastScore === undefined
    ? "text-ink-low"
    : loop.lastScore >= 0.85
      ? "text-accent-ok"
      : loop.lastScore >= 0.5
  ...
  return (
    <div className="bg-surface-2 border border-surface-3 rounded-md p-3 hover:border-surface-4 transition-colors">
      ...
      <div className="flex items-center gap-4 text-xs text-ink-mid mb-2">
        <span>
          iter: <span className="font-mono text-ink-high">{loop.iterations}</span>
        </span>
        {loop.lastScore !== undefined && (
          <span>
            score: <span className={`font-mono ${scoreColor}`}>{loop.lastScore.toFixed(2)}</span>
          </span>
        )}
        {loop.startedAt && (
          <span className="text-ink-low">
            started: <span className="font-mono">{formatTime(loop.startedAt)}</span>
          </span>
        )}
      </div>
      ...
      <div className="h-1.5 bg-surface-4 rounded overflow-hidden">
        <div
          className={`h-full transition-all ${
            loop.outcome === "exit_condition_met"
              ? "bg-accent-ok"
              : loop.outcome
                ? "bg-accent-err"
                : "bg-maestro-500"
