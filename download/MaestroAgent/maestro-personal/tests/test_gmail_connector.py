...
class TestGmailSendIntegration:
    ...
    def test_send_fails_closed_when_not_configured(self, no_gmail_env, tmp_path):
        """P0 honesty: when Gmail OAuth is not configured, resolve_draft must
        ...
        status=approved — a P0 fabrication that misled users into thinking
        their email was sent. The test below was updated to match the new
        honest behavior.
        """
        from maestro_personal_shell.connectors import ConnectorStore
        store = ConnectorStore(db_path=str(tmp_path / "test.db"))
