...
function TodayView({ onDraft, draftBusy }: { onDraft: (entity: string) => void; draftBusy: boolean }) {
  ...
  if (loading) return (
    <div className="space-y-8" aria-live="polite">
      <div>
        <div className="h-7 w-40 bg-gray-100 mb-2" />
        <div className="h-4 w-64 bg-gray-50" />
      </div>
      ...
      <div className="space-y-3">
        <div className="h-3 w-32 bg-gray-50" />
        {[1,2,3].map(i => (
          <div key={i} className="flex items-start gap-3 py-3">
            <div className="h-2 w-2 mt-1.5 bg-gray-100" />
            <div className="flex-1">
              <div className="h-4 w-full bg-gray-100 mb-1" />
...
function CommitmentsViewReal({ onDraft, draftBusy, searchQuery = '' }: { onDraft: (entity: string) => void; draftBusy: boolean; searchQuery?: string }) {
  ...
  if (loading) return (
    <div className="space-y-3" aria-live="polite">
      <div className="h-3 w-24 bg-gray-50 mb-4" />
      {[1,2,3].map(i => (
        <div key={i} className="flex items-start gap-3 py-3">
          <div className="h-2 w-2 mt-1.5 bg-gray-100" />
          <div className="flex-1">
...
function WhisperViewReal({ onDraft, draftBusy, searchQuery = '' }: { onDraft: (entity: string) => void; draftBusy: boolean; searchQuery?: string }) {
  ...
  if (loading) return (
    <div className="space-y-3" aria-live="polite">
      <div className="h-3 w-20 bg-gray-50 mb-4" />
      {[1,2,3].map(i => (
        <div key={i} className="flex items-start gap-3 py-3">
          <div className="h-2 w-2 mt-1.5 bg-gray-100" />
          <div className="flex-1">
