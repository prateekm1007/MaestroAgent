"""Gold scoring v3 — with P11 fix. Bulk-seed for user=bootstrap, fire 5
questions with LLM active. The P11 fix ensures the LLM is called even
when situation detection returns empty."""
...
tmp_db = tempfile.NamedTemporaryFile(suffix=".db", delete=False, prefix="gold_v3_")
tmp_db.close()
os.environ["MAESTRO_PERSONAL_DB"] = tmp_db.name
os.environ["MAESTRO_PERSONAL_TOKEN"] = "gold-v3-token"
os.environ["MAESTRO_PERSONAL_ALLOW_ARBITRARY_EMAIL"] = "1"
os.environ.pop("MAESTRO_PERSONAL_ENV", None)

# Bulk-seed directly into SQLite for user=bootstrap
...
import uvicorn
config = uvicorn.Config(personal_api.app, host="127.0.0.1", port=8790, log_level="error")
server = uvicorn.Server(config)
t = threading.Thread(target=server.run, daemon=True)
t.start()
for i in range(40):
    try:
        urllib.request.urlopen("http://127.0.0.1:8790/api/health", timeout=2)
...
print(f"\n[score] Firing {len(questions)} questions in parallel (P11 fix active)...")
...
def fire(q):
    ...
    try:
        ...
        return {"q": q["q"], "type": q["expected_type"], "m": m, "b": b,
                "llm": body.get("llm_active", False), "conf": body.get("confidence", 0),
                "ans": answer[:150], "elapsed": elapsed}
    except Exception as e:
        return {"q": q["q"], "type": q["expected_type"], "m": 0, "b": 0, "error": str(e)[:100], "elapsed": time.time() - t0}

results = [None] * len(questions)
with ThreadPoolExecutor(max_workers=3) as pool:
    f2i = {pool.submit(fire, q): i for i, q in enumerate(questions)}
    for f in as_completed(f2i):
        i = f2i[f]
        results[i] = f.result()
        r = results[i]
        print(f"  [{i+1}] {r['type']:15s} m={r['m']:.2f} b={r['b']:.2f} llm={r.get('llm')} ({r['elapsed']:.1f}s)")
...
print("\n" + "=" * 70)
print("  MEMORY GOLD SET — 5-QUESTION PARALLEL (P11 FIX + LLM ACTIVE)")
print("=" * 70)
