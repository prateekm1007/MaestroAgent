...
def build_markdown() -> str:
    ...
    lines.append("")
    lines.append("**Version:** 3.0 (2026-07-03) · **Commit:** `480f463` · **31 surfaces + ambient layer + CEO vision features**")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("- Simplified navigation: 4 sidebar items (Today, Ask, Memory, Decisions)")
    lines.append("")
    lines.append("**CEO Vision Features (new in v3.0):**")
    lines.append("- Preparation Engine: prepares for tomorrow's meetings before the user arrives")
    lines.append("- Whisper Memory: escalates after 3 ignores ('You've ignored this 3×')")
    lines.append("- Whisper Urgency: risk decays over time (14% → 42% over 5 days)")
    lines.append("- Collaborative Whispers: 'Engineering agrees, Legal disagrees'")
    lines.append("- Counterfactuals: 'If you merge today: 32% rollback, Monday: 14%'")
    lines.append("- Anticipation Engine: simulates tomorrow (meetings, risks, deadlines, blockers)")
    lines.append("- 300ms Response: whisper API cached for sub-second response")
        ...
        for i, w in enumerate(whisper_sample[:3], 1):
            ...
            if w.get("counterfactuals"):
                for cf in w["counterfactuals"][:2]:
                    lines.append(f"  What-if:   {cf['scenario']} → {cf['probability']} {cf['outcome']}")
            lines.append("")
        lines.append("```")
    else:
    ...
    lines.append("- **Ambient layer.** Whisper API returns 7-dimension format. 9 delivery surfaces. Outcome tracking.")
    ...
    lines.append("- **Whisper Memory + Urgency.** Escalates after 3 ignores. Risk decays over time (14% → 42%).")
    lines.append("- **Collaborative Whispers.** Shows team alignment ('Engineering agrees, Legal disagrees').")
    lines.append("- **Counterfactuals.** What-if scenarios ('Merge today: 32% rollback, Monday: 14%').")
    lines.append("- **Simplified navigation.** 4 sidebar items (Today, Ask, Memory, Decisions). 27 more via Ctrl+K.")
    lines.append("- **300ms response.** Whisper API cached for sub-second response.")
    ...
    lines.append("")
    lines.append("**Commit:** `480f463` · **Date:** 2026-07-03 · **Generated by execution, not by hand.**")
