...
def build_markdown() -> str:
    ...
    lines.append("")
    lines.append("**Version:** 4.0 (2026-07-03) · **Commit:** `4ef9e78` · **4 surfaces + ambient layer + conversational Ask**")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("| Urgency | Evidence-based risk language | 'Risk increasing — ignored for 5 days' |")
    lines.append("| Counterfactuals | What-if scenarios | 'Merge today: Higher risk — changes a component with rollback history' |")
    ...
    lines.append("")
    lines.append("### No Fake Precision (Constitution Principle)")
    lines.append("")
    lines.append("> **Maestro never invents precision. If a prediction cannot be empirically")
    lines.append("> calibrated and explained, it is expressed as evidence and reasoning")
    lines.append("> rather than a numerical probability.**")
    lines.append("")
    lines.append("- **Facts use numbers:** '11 customers raised pricing concerns'")
    lines.append("- **Patterns use evidence:** 'This deployment appears riskier than usual because it changes a component involved in two previous rollbacks'")
    lines.append("- **Predictions use plain language:** 'Higher deployment risk than similar changes'")
    lines.append("- **The ONE exception:** The Prediction Market, where probabilities are empirically calibrated against historical outcomes (Brier scores)")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("")
    lines.append("**Commit:** `4ef9e78` · **Date:** 2026-07-03 · **Generated by execution, not by hand.**")
