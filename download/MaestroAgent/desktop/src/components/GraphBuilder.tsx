  ...
  return function MaestroNode({ data, selected }: { data: NodeData; selected?: boolean }) {
    ...
    return (
      ...
      >
        ...
        {data.role && (
          <div className="text-[10px] text-ink-low mt-0.5">role: {data.role}</div>
        )}
        {data.goal && (
          <div className="text-[10px] text-ink-mid mt-0.5 truncate">→ {data.goal}</div>
        )}
        {kind === "loop" && (
        ...
        {kind === "hitl" && (
          <div className="text-[10px] text-accent-err mt-1">requires human approval</div>
        )}
      </div>
    );
  };
...
export default function GraphBuilder() {
  ...
  const exportGraph = useCallback(() => {
    ...
    a.href = url;
    a.download = `maestro-graph-${Date.now()}.json`;
    a.click();
  ...
  const importGraph = useCallback(() => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        console.warn("failed to import graph:", e);
      }
    };
    input.click();
  }, []);
        ...
        <div ref={wrapperRef} className="flex-1" onDragOver={onDragOver} onDrop={onDrop}>
          ...
          >
            ...
            <MiniMap
              ...
              nodeColor={(n) => {
                const meta = NODE_META[n.type as NodeKind];
