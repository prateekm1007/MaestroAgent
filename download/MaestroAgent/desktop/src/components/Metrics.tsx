...
import { DollarSign, Cpu, TrendingUp, AlertTriangle, Activity, Zap } from "lucide-react";
...
/**
 * Metrics dashboard — cost breakdown, token usage, agent performance.
 *
 * Pulls cost data from the sidecar's /api/costs/{run_id} endpoint and
 * augments with derived stats from the live event stream.
 */
export default function Metrics() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const result = await invoke<CostResponse>("get_cost_breakdown", { runId: currentRun.run_id });
        if (active) setCost(result);
      } catch (e) {
        console.warn("failed to load cost:", e);
      }
    };
    load();
  ...
  const loopIterations = events.filter((e) => e.type === "loop.iteration").length;
  const tokensIn = events
    .filter((e) => e.type === "llm.call.completed")
    ...
    <div className="h-full overflow-y-auto scrollbar-thin space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <StatCard
          ...
          color="text-accent-ok"
        />
        <StatCard
          icon={Zap}
          label="LLM Calls"
          value={String(llmCalls)}
          color="text-maestro-300"
        />
        <StatCard
          icon={Cpu}
          label="Tool Calls"
        ...
        <StatCard
          ...
          label="Errors"
          value={String(errors)}
          color={errors > 0 ? "text-accent-err" : "text-ink-mid"}
        />
      </div>
      ...
      <div className="panel">
        <div className="panel-header flex items-center justify-between">
          ...
          <span className="text-xs text-ink-low font-mono normal-case">
            {cost?.breakdown.length ?? 0} providers
          </span>
        </div>
              ...
              <tbody>
                {cost.breakdown.map((b, i) => (
                  <tr key={i} className="border-b border-surface-2 hover:bg-surface-2">
                    ...
                    <td className="px-4 py-2 text-right font-mono text-xs text-accent-ok">
                      ${b.cost_usd.toFixed(4)}
                    </td>
                    <td className="px-4 py-2">
                      <div className="h-1.5 bg-surface-4 rounded overflow-hidden w-24">
                        <div
                          className="h-full bg-maestro-500"
                          style={{
                            width: `${(b.cost_usd / Math.max(cost.total_usd, 0.0001)) * 100}%`,
                          }}
                        />
                      </div>
                    </td>
      ...
      <div className="panel">
        ...
        <div className="p-4">
          <IterationHistogram events={events} />
        </div>
      </div>
    </div>
  );
...
function IterationHistogram({ events }: { events: ReturnType<typeof useAppStore.getState>["events"] }) {
  // Build a histogram of loop iterations by loop_id.
  ...
  if (loops.length === 0) {
    return <div className="text-sm text-ink-low">No loop iterations recorded.</div>;
  }
        ...
        return (
          <div key={id}>
            <div className="flex justify-between text-xs text-ink-mid mb-1">
              ...
              <span>{scores.length} iters</span>
            </div>
            <div className="flex items-end gap-0.5 h-12">
              {scores.map((s, i) => (
                <div
                  key={i}
                  className="flex-1 bg-maestro-500 hover:bg-maestro-400 transition-colors rounded-t"
