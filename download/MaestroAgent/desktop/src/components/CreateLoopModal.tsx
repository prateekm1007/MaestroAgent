...
/**
 * CreateLoopModal — define a new verifiable loop and attach it to the run.
 *
 * The loop wraps a body agent and runs until its exit condition is met.
 * Exit conditions: tests pass, metric threshold, critic score, or callable.
 */
export default function CreateLoopModal() {
  ...
  const handleSubmit = async () => {
    if (!loopId.trim()) {
      setError("Loop ID is required.");
      return;
    }
    if (!bodyAgentId) {
      setError("Body agent is required.");
      return;
    }

    const exitConfig: Record<string, unknown> = {};
    if (exitKind === "tests") {
      exitConfig.command = testCommand;
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. fix_until_tests_pass"
            ...
            <div>
              ...
              <select
                value={bodyAgentId}
                onChange={(e) => setBodyAgentId(e.target.value)}
            ...
            {exitKind === "metric" && (
              <div className="grid grid-cols-3 gap-2">
                ...
                <div>
                  ...
                  <select
                    value={metricComparator}
                    onChange={(e) => setMetricComparator(e.target.value)}
                    className="input w-full"
                  >
                    <option value=">=">≥</option>
                    <option value="≤">≤</option>
            ...
            {exitKind === "critic" && (
              <div className="space-y-2">
                <div>
                  ...
                  <textarea
                    value={criticRubric}
                    onChange={(e) => setCriticRubric(e.target.value)}
                    placeholder="The output must..."
                    rows={2}
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                value={onExceed}
                onChange={(e) => setOnExceed(e.target.value as typeof onExceed)}
                className="input w-full"
              >
