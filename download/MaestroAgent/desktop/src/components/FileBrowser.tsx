...
/**
 * File browser — browse the workspace produced by the active run.
 *
 * For v0.1 we render a static tree with the workspace path. v0.2 will
 * use the Tauri fs plugin to actually list the workspace directory.
 */
        ...
        <ul>
          {tree.map((entry) => (
            <li key={entry.path}>
              <FileRow
                entry={entry}
                depth={entry.path.split("/").length - 2}
                expanded={expanded[entry.path] || false}
                selected={selected === entry.path}
                onToggle={() =>
                  setExpanded((e) => ({ ...e, [entry.path]: !e[entry.path] }))
                }
...
function FileRow({
  ...
  return (
    <div
      className={`flex items-center gap-1.5 py-1 px-2 rounded cursor-pointer hover:bg-surface-2 ${
    ...
    >
      {entry.is_dir ? (
        expanded ? (
          <ChevronDown className="w-3 h-3 text-ink-low" />
        ) : (
          <ChevronRight className="w-3 h-3 text-ink-low" />
        )
      ) : (
        <span className="w-3" />
      )}
      {entry.is_dir ? (
        <FolderTree className="w-3.5 h-3.5 text-accent-warn" />
      ) : (
        <File className="w-3.5 h-3.5 text-accent-info" />
      )}
