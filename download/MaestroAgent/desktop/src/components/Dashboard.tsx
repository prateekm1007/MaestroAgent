 ...
 *
 * Shows:
 * - Run summary card (status, cost, iteration, current node, error)
 * - Quick stats (events/sec, tokens, errors, providers)
 * - Live event stream from the active run's WebSocket
 */
      ...
      <div className="col-span-8 flex flex-col gap-4 overflow-hidden">
        ...
        <div className="panel flex-1 flex flex-col overflow-hidden">
          <div className="panel-header flex items-center justify-between">
            ...
            <span className="text-xs text-ink-low normal-case font-mono">
          ...
          <div className="p-4 overflow-y-auto scrollbar-thin text-sm space-y-3">
            {currentRun ? (
              <>
                ...
                <Row
                  label="Cost"
                  value={`$${(currentRun.cost_usd ?? 0).toFixed(4)}`}
                />
              ...
              <div className="text-ink-low text-xs">
                No active run. Start one from Templates.
              </div>
            )}
          </div>
        </div>
...
function Row({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex justify-between gap-2">
      ...
      <span className={`text-ink-high text-right truncate ${mono ? "font-mono text-xs" : ""}`}>
        {value}
      </span>
    </div>
  );
