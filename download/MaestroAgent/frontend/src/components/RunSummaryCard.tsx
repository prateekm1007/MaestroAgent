  ...
  return (
    <div className="panel p-4">
      ...
      <div className="grid grid-cols-4 gap-3 text-sm">
        ...
        <Metric icon={AlertTriangle} label="Errors" value={run.error ? "1" : "0"} danger={!!run.error} />
      </div>
      {run.error && (
...
function Metric({ icon: Icon, label, value, danger }: {
  icon: typeof DollarSign; label: string; value: string; danger?: boolean;
}) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className={`font-mono text-lg ${danger ? "text-accent-err" : "text-ink-high"}`}>{value}</div>
    </div>
  );
}
