...
import {
  LayoutTemplate, Zap, Search, Star, Download, Play, Code2, FlaskConical,
  Rocket, ShoppingCart,
} from "lucide-react";
    ...
    <div className="h-full overflow-y-auto scrollbar-thin">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-ink-low">One-click workflows. Pick a template, set a goal, run.</p>
        </div>
        <div className="relative">
          ...
          <input type="text" placeholder="Search templates..." value={query}
            onChange={(e) => setQuery(e.target.value)} className="input pl-8 w-64" />
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold text-ink-high uppercase tracking-wide">Built-in Workflows</h3>
          <span className="text-xs text-ink-low">({filtered.length})</span>
        </div>
        {filtered.length === 0 ? (
          <div className="panel p-8 text-center text-sm text-ink-low">
            {templates.length === 0
              ? "Loading templates... (make sure the backend is running)"
            ...
            {filtered.map((t) => {
              ...
              return (
                <button key={t.name}
                  ...
                  onDoubleClick={() => { setSelectedTemplate(t.name); openStartRunModal(); }}
                ...
                >
                  <div className="flex items-start gap-3 mb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-ink-high truncate">{t.name}</div>
                      <div className="text-[10px] text-ink-low font-mono">{t.path.split("/").pop()}</div>
                    </div>
                  </div>
                  <p className="text-xs text-ink-mid line-clamp-2 mb-3">{t.description || "No description available."}</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 text-[10px] text-ink-low">
                      <Star className="w-3 h-3" /><span>built-in</span>
                    </div>
      ...
      {selectedTemplate && (
        <div className="sticky bottom-0 left-0 right-0 bg-surface-1 border-t border-surface-3 p-3 flex items-center justify-between">
          ...
          <button onClick={openStartRunModal} className="btn-primary">
            <Play className="w-3.5 h-3.5" /> Configure & Run
          </button>
        </div>
      )}
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold text-ink-high uppercase tracking-wide">Marketplace</h3>
          <span className="badge-warn">coming in v1.0</span>
        </div>
        <div className="panel p-6 text-center">
          <Download className="w-8 h-8 text-ink-low mx-auto mb-2" />
          <p className="text-sm text-ink-mid mb-1">Community templates & agent marketplace</p>
          <p className="text-xs text-ink-low">
            Browse, install, and share agent swarms with one click. Signed plugins, cost previews, sandboxed trials.
          </p>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold text-ink-high uppercase tracking-wide">Featured Swarms</h3>
        </div>
