...
export default function StartRunModal() {
  ...
  useEffect(() => {
    if (!isOpen) return;
    api.health().then((h) => setProviders(h.providers)).catch(() => setProviders(["ollama"]));
  }, [isOpen]);
  ...
  const handleLaunch = async () => {
    ...
    try {
      await startRun({
        template, goal: goal.trim(), max_cost_usd: maxCost,
        default_provider: provider, default_model: model || undefined,
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-surface-1 border border-surface-3 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto scrollbar-thin">
        ...
        <div className="p-6 space-y-5">
          <div>
            <label className="block text-xs text-ink-low uppercase tracking-wide mb-1.5">Template</label>
            <select value={template} onChange={(e) => setTemplate(e.target.value)} className="input w-full">
              <option value="blank">blank (no-op)</option>
              {templates.map((t) => (
                <option key={t.name} value={t.name}>{t.name} — {t.description.slice(0, 60)}</option>
              ))}
            </select>
          </div>
          <div>
            <label className="block text-xs text-ink-low uppercase tracking-wide mb-1.5">Goal</label>
            <div className="relative">
              <textarea value={goal} onChange={(e) => setGoal(e.target.value)}
                placeholder="e.g. Build a notes SaaS with auth + Stripe"
                rows={3} className="input w-full resize-none pr-12" />
              {supported && (
                <button onClick={listening ? stop : start}
                  className={`absolute right-2 top-2 p-1.5 rounded-md transition-colors ${
                    listening ? "bg-accent-err/20 text-accent-err animate-pulse"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="flex items-center justify-between text-xs text-ink-low uppercase tracking-wide mb-1.5">
                <span className="flex items-center gap-1"><DollarSign className="w-3 h-3" /> Max Cost</span>
                <span className="text-accent-ok font-mono normal-case">${maxCost.toFixed(2)}</span>
              </label>
              <input type="range" min={0.5} max={100} step={0.5} value={maxCost}
                onChange={(e) => setMaxCost(Number(e.target.value))} className="w-full accent-maestro-500" />
            ...
            <div>
              <label className="flex items-center justify-between text-xs text-ink-low uppercase tracking-wide mb-1.5">
                <span className="flex items-center gap-1"><Zap className="w-3 h-3" /> Max Iterations</span>
              ...
              <input type="range" min={5} max={500} step={5} value={maxIterations}
                onChange={(e) => setMaxIterations(Number(e.target.value))} className="w-full accent-maestro-500" />
              <div className="flex justify-between text-[10px] text-ink-low mt-0.5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-ink-low uppercase tracking-wide mb-1.5">
                <Cpu className="w-3 h-3 inline mr-1" />Provider
              </label>
              <select value={provider} onChange={(e) => setProvider(e.target.value)} className="input w-full">
                {(providers.length ? providers : ["ollama"]).map((p) => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs text-ink-low uppercase tracking-wide mb-1.5">Model (optional)</label>
              <input type="text" value={model} onChange={(e) => setModel(e.target.value)}
                placeholder="default for provider" className="input w-full" />
            </div>
          </div>
          {error && (
            <div className="text-xs text-accent-err p-2 bg-accent-err/10 rounded font-mono">{error}</div>
          )}
        </div>
