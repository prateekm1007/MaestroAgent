...
class OrganizationalWhisper:
    ...
    def _context_whispers(self, context: str, entity: str) -> list[dict[str, Any]]:
        ...
        W1.5 fix: Context-only whispers with no signal evidence have been
        removed. These were generic reminders ("Review relationship memory")
        presented as if they were organizational intelligence. They are not —
        they are UI prompts with no evidence backing.

        If entity-specific whispers from real signals are needed, they are
        already generated by _entity_whispers(). This method now returns
        empty — no fake evidence.
        """
        return []
