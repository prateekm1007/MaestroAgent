            ...
            ${buckets.map(b => `
              <div class="flex items-center gap-2 text-[10px]">
                ...
                <div class="flex-1 h-3 bg-white/[0.04] rounded overflow-hidden relative">
                  <div class="h-full bg-brand-cyan/40" class="auto-wbexpectedrate100p"></div>
                  <div class="absolute top-0 h-full bg-brand-violet/60" class="auto-wbactualrate100p"></div>
                </div>
      ...
      ${trend.length > 0 ? `
        <div>
          ...
          <div class="flex items-end gap-1 h-12">
            ${trend.map(t => `<div class="flex-1 bg-brand-cyan/40 rounded-t" class="auto-htaccuracy100" title="${t.week}: ${(t.accuracy*100).toFixed(0)}% (${t.predictions} predictions)"></div>`).join('')}
          </div>
        </div>
      ` : ''}
...
function renderRecCard(r) {
  ...
  return `<div class="card ${r.urgency === 'urgent' ? 'urgent' : ''} mb-3 cursor-pointer" role="button" tabindex="0" onclick="openDrilldown('recommendation', '${escapeJs(r.title)}')">
    ...
    <div class="flex items-center gap-3 text-[10px] text-fg-500 mt-2">
      <div class="conf-bar" class="auto-w120"><div class="conf-bar-track"><div class="conf-bar-fill" class="auto-wrconfidence100p"></div></div><span class="text-brand-cyan font-bold">${formatConfidenceWithWhy(r.confidence, { entity: 'recommendation', title: r.title })}</span></div>
      <span>·</span>
...
function renderLawCard(l) {
  ...
  return `<div class="card mb-3 cursor-pointer" role="button" tabindex="0" onclick="openDrilldown('law', '${escapeJs(l.code)}')">
    ...
    <div class="flex items-center gap-3 text-[10px] text-fg-500 mt-2">
      <div class="conf-bar" class="auto-w120"><div class="conf-bar-track"><div class="conf-bar-fill" class="auto-wlconfidence100p"></div></div><span class="text-brand-cyan font-bold">${formatConfidenceWithWhy(l.confidence, { entity: 'law', title: l.statement })}</span></div>
      <span>·</span>
      ...
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-4 pt-3 border-t border-white/[0.05]">
          ...
          <div>
            ...
            <div class="conf-bar mt-1"><div class="conf-bar-track"><div class="conf-bar-fill" class="auto-wsconfidence100p"></div></div><span class="text-brand-cyan font-bold">${formatConfidence(s.confidence)}</span></div>
          </div>
        </div>
    ...
    result.innerHTML = `
      <div class="space-y-3">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <div class="text-[10px] uppercase text-fg-500">Confidence</div>
            <div class="conf-bar mt-1"><div class="conf-bar-track"><div class="conf-bar-fill" class="auto-wdataconfidence100p"></div></div><span class="text-brand-cyan font-bold">${formatConfidence(data.confidence)}</span></div>
          </div>
        </div>
