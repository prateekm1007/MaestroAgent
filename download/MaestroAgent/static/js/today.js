          ...
          <div class="brief-item b-u-p100">
            <div class="b-flex-u-6">
              ...
              <div class="b-flex-gap4">
                ${c.to_whom ? `<button class="ds-btn ds-btn-ghost ds-btn-small b-fs11-ws" onclick="showTrajectoryPanel(${i}, ${JSON.stringify(c.to_whom).replace(/"/g, '&quot;')})">Trajectory</button>` : ''}
...
async function showTrajectoryPanel(idx, entity) {
  const el = document.getElementById(`commitment-trajectory-${idx}`);
  if (!el) return;
  // Toggle off if already shown (same UX as showInlineWhy)
  if (el.innerHTML.trim()) {
    el.innerHTML = '';
    return;
  }
  ...
  try {
    ...
    el.innerHTML = `
      <div class="b-p812-bg b-mt4-u" style="border-left:3px solid var(--accent)">
        ...
        ${recHtml}
        ${evHtml}
        <button class="ds-btn ds-btn-ghost ds-btn-small fs-11 b-mt4-u" onclick="document.getElementById('commitment-trajectory-${idx}').innerHTML=''">Close</button>
      </div>
    `;
  } catch (e) {
    el.innerHTML = `<div class="ds-error fs-11">Failed: ${escapeHtml(e.message)}</div>`;
